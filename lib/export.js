...
function getExportFunc(R) {
    ...
    var serializer = {
        'text': function text(node) {
            ...
            map(textLines, function (text, line) {
                ...
                tags.push(tag('text', attrs, node.matrix, tag('tspan', { dy: computeTSpanDy(style.font.size, line + 1, totalLines) }, null, text.replace(/&/g, "&amp;"))));
            });
    ...
    var toSVG = function toSVG() {
        ...
        var paper = this,
            ...
            svg = '<svg style="overflow: visible; position: relative;" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="' + paper.canvas.viewBox.baseVal.width + '" version="1.1" height="' + paper.canvas.viewBox.baseVal.height + '">' + '<defs>' + defs + '</defs>';
            ...
            for (var i in node.attrs) {
                ...
                switch (i) {
                    ...
                    case 'fill':
                        ...
                        if (node.attrs.gradient) continue;
                        break;
                    case 'gradient':
                        //radial gradient
                        var id = node.id;
                        ...
                        gradient = String(gradient).replace(R._radial_gradient, function (all, _fx, _fy) {
                            ...
                            if (_fx && _fy) {
                                ...
                                fy = parseFloat(_fy);
                                var dir = (fy > .5) * 2 - 1;
                                Math.pow(fx - .5, 2) + Math.pow(fy - .5, 2) > .25 && (fy = Math.sqrt(.25 - Math.pow(fx - .5, 2)) * dir + .5) && fy != .5 && (fy = fy.toFixed(5) - 1e-5 * dir);
                            }
                            return '';
                        });
                        ...
                        if (node.attrs.gradient.match(/^r/g)) {
                            ...
                            for (var di = 0; di < dots.length; di++) {
                                ...
                                if (dots[di].offset) offset = dots[di].offset;
                ...
                if (name) {
                    if (value) attrs += ' ' + name + '="' + escapeXML(value) + '"';else attrs += ' ' + name + '="' + escapeXML(node.attrs[i].toString()) + '"';
                }
            }
